...
struct VagueModel
	...
	VagueModel(λ, coverage, scale_points, prior) = let
		...
		new(λ, coverage, scale_points, probabilities, θ_probabilities)
	end
end
...
If the data shows a clear division into clusters, it is possible that an alternative interpretation emerges. You interpret the adjective on the level of clusters (which groups would you call "big", "expensive", etc.). To make a judgement for an individual object, you infer which cluster it could belong to.

So we have a speaker function on the level of objects, $S_O : O \mapsto [0,1]$, which  says if you would use the adjective for a degree $x$ on the scale.

Then assume that the prior distribution $G = \{G_1, G_2, \dots\}$ is a gaussian mixture. For a point $x$, $P(x \sim G_i | x \sim G )$ represents the probability that a degree $x$ was generated by $G_i$, given the observations that $x$ was generated by $G$. In other words, this is the likelihood that $G_i$ is the underlying generator of $x$.

We have a group-level speaker function, which applies the adjective to the components of $G$, $S_G : G \mapsto [0,1]$.

Then

$S_O(x) = \sum_{G_i \in G} P(x \sim G_i | x \sim G ) \cdot S_G(G_i)$

The group-level speaker function $S_G$ could function in the same way as the vague model above, but because we only use bimodal distributions, we can assume that $S_G$ is a binary function. We just assign $1$ to the higher group and $0$ to the lower group.
...
# ╔═╡ 25bdb704-8a8b-4b8f-ad5f-c6fb1bf5ea99
md"""
Per illustration, here is the vague model with a bimodal distribution:
"""

# ╔═╡ 62a3dda6-0ff8-4edb-9907-735659a7b2a9
example_bimodal = MixtureModel([Normal(30, 5), Normal(70, 5)]) ;

# ╔═╡ afe66ef8-3d55-481a-94c3-7c06b4aeed53
begin
	bimodal_example_λ = 150
	bimodal_example_c = -0.01
end ;

# ╔═╡ e2162f3e-95cd-475e-96b0-76272d10c800
md"Here is the group-level speaker function:"

# ╔═╡ 59f6abde-4c06-44e1-8528-e7306047bfa2
plot_illustration_bimodal_grouplevel = let
	speaker(x) = group_level_use_adjective(x, example_bimodal)
	
	#infer threshold probabilities (the derivative of the speaker function)
	function θ_probability(x)
		if x == first(example_scale_points)
			speaker(x)
		else
			speaker(x) - speaker(x - step(example_scale_points))
		end
	end
...
# ╔═╡ 10299aa0-eefd-41b8-b634-88e1a05e9676
md"""
When we put this together, we can imagine that a speaker uses a vague interpretation of the adjective, but when they encounter a bimodal distribution, they can either use this group-level interpretation, or continue to use the object-level vague model.

Let's introduce a parameter $\alpha$ that determines how likely people are to use the vague interpretation if it is available to them.
"""

# ╔═╡ 6914b517-a3c8-481b-85ba-f288f8792a17
struct CompositeModel
	...
	#constructor
	CompositeModel(λ, coverage, α, scale_points, prior) = let
		vague_model = VagueModel(λ, coverage, scale_points, prior)
		
		probabilities = vague_model.probabilities
		θ_probabilities = vague_model.θ_probabilities
		
		new(λ, coverage, α, scale_points, prior, probabilities, θ_probabilities)
	end
end

# ╔═╡ 07988ca2-b3ad-48a9-9b9a-4d8a5713bc49
function use_adjective(degree, model::CompositeModel)
	#two speaker functions
	vague_speaker(degree) = use_adjective(degree, 
		model.θ_probabilities, model.scale_points)
	
	group_level_speaker(degree) = group_level_use_adjective(degree, model.prior)
	
	if typeof(model.prior) <: MixtureModel
		vague_speaker(degree) * (1 - model.α) + group_level_speaker(degree) * model.α
	else
		vague_speaker(degree)
	end
end
...
# ╔═╡ 6a27e846-d96e-4859-ad7b-363abc44034a
plot_illustration_unimodal = let
	vague_model = VagueModel(bimodal_example_λ, bimodal_example_c,
		example_scale_points, Normal(50, 10))
	
	p_prior = plot(example_scale_points,
		x -> vague_model.probabilities[x][:density],
		legend = :none,
		title = "prior probability",
		ylabel = "P(degree)", xlabel = "degree",
		fill = 0, fillalpha = 0.75,
	...
	p_threshold = plot(example_scale_points,
		vague_model.θ_probabilities,
		legend = :none,
		title = "threshold probability",
		ylabel = "P_θ(degree)", xlabel = "degree",
		fill = 0, fillalpha = 0.75,
	...
	p_speaker = plot(example_scale_points,
...
plot_illustration_bimodal_vague = let
	vague_model = VagueModel(bimodal_example_λ, bimodal_example_c,
		example_scale_points, example_bimodal)
...
if "figures" in readdir(root)
	plots_filenames = [
		(plot_illustration_unimodal, "model_illustration_unimodal"),
		(plot_illustration_bimodal_vague, "model_illustration_bimodal_vague"),
		(plot_illustration_bimodal_grouplevel, "model_illustration_bimodal_groups")
	]
	
	for (plot, filename) in plots_filenames
		path = root * "/figures/$(filename).pdf"
		savefig(plot, path)
	end
